...
def convert(millisecond):
    ...
    millisecond_left = millisecond % minute_in_millisecond
...
def main_get():
        return render_template('index.html', developer_name =' Ahmet Cansever', not_valid = False)

@app.route('/', methods=['POST'])
def main_post():
    alpha = request.form['number']
    if not alpha.isdecimal():
        return render_template('index.html', developer_name = 'Ahmet Cansever', not_valid = True)
    if not (0 < int(alpha)):
        return render_template('index.html', developer_name = 'Ahmet Cansever', not_valid = True)
    return render_template('result.html', developer_name=' Ahmet Cansever', milliseconds = alpha, result = convert(int(alpha)) )
